...
def center_at_points(
    ...
    delta_mapped = delta @ jnp.linalg.pinv(cell_size)
    delta -= jnp.round(delta_mapped) @ cell_size
    radius = jnp.where(types < 0, jnp.inf, _sqrt(jnp.sum(delta**2, axis=-1)))
    return delta, radius, types
...
def create_sc_center_functions(sc_a: int, sc_b: int, sc_c: int):
    ...
    def sc_center_at_atoms(coordinates, types, cell_size):
        ...
        delta_mapped = delta @ jnp.linalg.pinv(super_cell)
        delta -= jnp.round(delta_mapped) @ super_cell
        radius = jnp.where(
            super_types < 0, jnp.inf, _sqrt(jnp.sum(delta**2, axis=-1))
        )
